...
| **Flashcards** | ~274 cards (conversation-mined vocab + common-adjectives, comparisons, and connectors reference decks). Cards can carry multiple tags, and the category filter is toggle chips — mix categories (union) or drill into one. Add cards from the UI one at a time, or paste a whole `front – back` list at once with an optional `(tag)` header per block — no file editing needed. |
