...
<style>
  ...
  * { box-sizing:border-box; }
  ...
  /* One ~40px row: back arrow, the count you actually glance at, undo, and
     everything else behind "⋯". It was 170px of <h1> + a stats sentence +
     two rows of buttons before the card even started, on a view whose whole
     job is to show one card. */
  .top { display:flex; align-items:center; gap:8px; margin-bottom:14px; }
  a.back { color:var(--muted); font-size:1.05rem; line-height:1; text-decoration:none;
    padding:5px 4px; }
  a.back:hover { color:var(--fg); }
  .stats { color:var(--muted); font-size:0.8rem; }
  .top .stats { flex:1; min-width:0; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
  button.icon { position:relative; font-size:0.95rem; line-height:1; padding:6px 8px;
    border-radius:8px; border:1px solid transparent; background:none; color:var(--muted);
    cursor:pointer; }
  /* An active tag or search filter used to announce itself as "Filter · 2" in
     the top bar. That button lives in the menu now, so the menu button carries
     the mark — without it you can be studying 12 of 997 cards with nothing on
     screen saying so. */
  button.icon.marked::after { content:''; position:absolute; top:2px; right:2px;
    width:6px; height:6px; border-radius:50%; background:var(--accent); }
  button.icon:hover:not(:disabled) { border-color:var(--line); color:var(--fg); }
  button.icon:disabled { opacity:0.3; cursor:default; }
  button.icon.active { border-color:var(--accent); color:var(--accent); }
  /* The overflow menu. Everything in here is either a session-boundary action
     (reset, restore) or a set-once preference (clip speed, auto-compare) —
     nothing you touch while actually working through a card. */
  .menu { display:flex; flex-direction:column; gap:10px; }
  .menu .row { display:flex; flex-wrap:wrap; gap:8px; align-items:center; }
  .menu .row.sep { border-top:1px solid var(--line); margin-top:2px; padding-top:10px; }
  .menu .meta { color:var(--muted); font-size:0.72rem; display:flex; gap:10px;
  ...
  /* min-height was 220px, which wrapped a 3-word front in a box twice the
     size of its content. The floor is now just enough to keep short cards
     from looking like a toast; the bottom padding is the audio bar's lane. */
  #card { position:relative; border:1px solid var(--line); border-radius:14px; background:var(--card);
    ...
    padding:22px 18px 42px; cursor:pointer; margin:0 0 14px; user-select:none; }
  /* showCard() rewrites this wrapper rather than the whole card, so the audio
     bar below survives the redraw with its listeners intact. */
  #cardBody { width:100%; }
  ...
  #card .back .en { font-size:1.25rem; font-weight:600; }
  ...
  /* Collapsed to its label by default: 107px median (224px worst) of quoted
     speech that pushed the rate buttons off a phone screen. The play control
     stays on the header row, so the family's actual voice is still one tap
     away without expanding anything. */
  #card .back .heard .heardTop { display:flex; align-items:center;
    justify-content:center; gap:8px; }
  #card .back .heard .heardToggle { font-size:0.66rem; letter-spacing:0.06em;
    text-transform:uppercase; opacity:0.75; background:none; border:none;
    color:inherit; cursor:pointer; padding:2px 0; font-family:inherit; }
  #card .back .heard .heardToggle::after { content:' \\25BE'; }
  #card .back .heard.open .heardToggle::after { content:' \\25B4'; }
  #card .back .heard .heardToggle:hover { opacity:1; }
  #card .back .heard .heardBody { margin-top:6px; }
  #card .back .heard .heardBody[hidden] { display:none; }
  #card .back .heard .heardPlay { padding:2px 9px;
    font-size:0.74rem; color:var(--muted); background:none;
    border:1px solid var(--line); border-radius:8px; cursor:pointer; }
  #card .back .heard .heardPlay:hover { color:var(--fg); }
  /* Hear it / Say it / Compare live inside the card now, as one icon row
     pinned to its bottom edge. They used to be 85px of labelled buttons
     stacked between the card and the rating buttons — on the same axis your
     thumb travels, in the one gap that should be empty. The order still
     encodes the method: hear it, say it, then hear both back to back. */
  .cardBar { position:absolute; left:0; right:0; bottom:9px; display:flex;
    align-items:center; justify-content:center; gap:6px; flex-wrap:wrap; }
  .cardBar[hidden] { display:none; }
  .cardBar button { font-size:0.8rem; line-height:1; padding:6px 10px; border-radius:8px;
    border:1px solid var(--line); background:var(--bg); color:var(--muted); cursor:pointer; }
  .cardBar button:hover:not(:disabled) { border-color:var(--accent); color:var(--accent); }
  .cardBar button:disabled { opacity:0.35; cursor:default; }
  .cardBar button[hidden] { display:none; }
  .cardBar button.recording { border-color:var(--again); color:var(--again); }
  .cardBar button.on { border-color:var(--accent); color:var(--accent); }
  /* Which voice you're hearing is a real distinction and stays — but as a
     one-word tag. The full sentence it replaced ("example sentence · studio
     voice, not the family") wrapped to two lines on a phone; it survives as
     the play button's tooltip. */
  .cardBar .src { color:var(--muted); font-size:0.66rem; letter-spacing:0.05em;
    text-transform:uppercase; opacity:0.7; }
  .cardBar .src[hidden] { display:none; }
  .cardBar .src.model { color:var(--accent); opacity:1; }
  .cardBar .src.you { color:var(--good); opacity:1; }
  /* "Listen for" is per-card, so it keeps its text — but behind the ⓘ in
     the bar rather than as a permanently-rendered 33px header. */
  .sayTipBox { max-width:460px; margin:-4px auto 12px; border-radius:8px; background:var(--card);
    border:1px solid var(--line); color:var(--muted); font-size:0.78rem; line-height:1.5;
    text-align:left; padding:8px 12px; }
  .sayTipBox[hidden] { display:none; }
  .sayTipBox b { color:var(--accent); }
  .autoCmp { display:inline-flex; align-items:center; gap:5px; cursor:pointer;
    color:var(--muted); font-size:0.75rem; user-select:none; }
  .autoCmp input { margin:0; cursor:pointer; }
__TIPS_CSS__
__OFF_FILTER_CSS__
  .hint { text-align:center; color:var(--muted); font-size:0.78rem; margin:0 0 16px; }
...
<div class="wrap">
  <div class="top">
    <a class="back" href="index.html" title="All conversations">&larr;</a>
    <span class="stats" id="stats"></span>
    <button class="icon" id="undoBtn" title="Go back (z)" disabled>&#8630;</button>
    <button class="icon" id="moreBtn" title="More" aria-haspopup="true">&#8943;</button>
  </div>
  <div class="panel" id="morePanel" hidden>
    <div class="menu">
      <div class="row">
      ...
      <div class="row" id="speedGroup" title="Speed of generated/volunteer clips — real family audio is unaffected">
        <span class="stats">Clip speed</span>
      ...
      <div class="row">
        <label class="autoCmp" title="Play the model-and-you comparison automatically as soon as you stop recording">
          <input type="checkbox" id="autoCmpBox"> auto-compare after recording
        </label>
      </div>
      <div class="row sep">
        <button class="plain danger" id="removeBtn">Remove card</button>
...
atau – or"></textarea>
  ...
  <div id="card">
    <div id="cardBody"><div class="front"></div><div class="back"><div class="q"></div><div class="en"></div><div class="ex"></div><div class="heard"></div></div></div>
    <div class="cardBar" id="cardBar">
      <button type="button" id="playBtn" title="Hear it (p)">&#128266;</button>
      <button type="button" id="recBtn" title="Say it" hidden>&#127908;</button>
      <button type="button" id="cmpBtn" title="Compare" hidden>&#8646;</button>
      <button type="button" id="sayTipBtn" title="What to listen for" hidden>&#9432;</button>
      <span class="src" id="playNote"></span>
      <span class="src" id="sayNote"></span>
    </div>
  </div>
  <div class="sayTipBox" id="sayTipBox" hidden></div>
...
function updateFilterChrome() {
  ...
  more.classList.toggle('marked', n > 0);
  more.title = n
    ? `More — filtered to ${pool.length} of ${DECK.length} cards`
    : 'More';
}
...
function showCard(card) {
  ...
  flipped = false;
  cardEl.className = '';
  // Only the body is redrawn — the audio bar is a sibling, so it keeps its
  // listeners across cards. The tag line is gone: it's metadata you already
  // have in Browse and in the filter chips, not something you're recalling.
  cardBodyEl.innerHTML = '<div class="front"></div><div class="back"><div class="q"></div>' +
    '<div class="en"></div><div class="ex"></div><div class="heard"></div></div>';
  cardBodyEl.querySelector('.front').textContent = current.front;
  cardBodyEl.querySelector('.q').textContent = current.front;
  cardBodyEl.querySelector('.en').textContent = current.back;
  ...
  if (current.exampleHtml) {
    cardBodyEl.querySelector('.ex').innerHTML = current.exampleHtml +
  ...
  if (current.sentenceHtml) {
    ...
    const heardEl = cardBodyEl.querySelector('.heard');
    heardEl.innerHTML =
      '<div class="heardTop">' +
        '<button type="button" class="heardToggle">Heard in the recording</button>' +
        (current.audio && modelSrc()
          ? '<button type="button" class="heardPlay" title="Play the family clip">&#9654;</button>' : '') +
      '</div>' +
      '<div class="heardBody">' + current.sentenceHtml +
        (current.exTranslation ? `<div class="exEn">${escapeHtml(current.exTranslation)}</div>` : '') +
      '</div>';
    applyHeardOpen(heardEl);
    // Both controls stop propagation — the card itself flips on click.
    heardEl.querySelector('.heardToggle').addEventListener('click', e => {
      e.stopPropagation();
      setHeardOpen(!heardOpen());
      applyHeardOpen(heardEl);
    });
    const hp = heardEl.querySelector('.heardPlay');
...
function renderEmpty(msg) {
  ...
  stopAudio();
  updatePlayRow();
  cardBodyEl.innerHTML = `<div class="empty">${msg}</div>`;
  rateRow.hidden = true;
}
...
function renderAllCaughtUp() {
  ...
  cardBodyEl.innerHTML = `<div class="empty">🎉 All caught up!<div class="sub">Next review in ${srsFmtDue(nextDue)}${in24h ? ` · ${in24h} due within 24h` : ''}.</div>` +
...
function renderStats() {
  ...
  // The header carries only the number you actually glance at between cards.
  // The full sentence it used to spell out is the tooltip.
  const bits = [];
  if (dueReviews) bits.push(dueReviews + ' due');
  if (fresh) bits.push(fresh + ' new');
  const statsEl = document.getElementById('stats');
  statsEl.textContent = bits.length ? bits.join(' · ') : mature + ' mastered';
  statsEl.title =
...
function flip() {
  ...
  updatePlayRow();          // which voice the play button offers changes on flip
  updateSayRow();
...
const PANEL_IDS = ['morePanel', 'filterPanel', 'browsePanel', 'addForm', 'bulkForm'];
...
function closePanels(except) {
  ...
  document.getElementById('moreBtn')
    .classList.toggle('active', except === 'morePanel');
}
...
function togglePanel(id) {
  ...
  document.getElementById('moreBtn')
    .classList.toggle('active', id === 'morePanel' && willOpen);
  return willOpen;
}
...
// Which voice you're about to hear is a distinction worth keeping — the whole
// point of the 2026-08 retool was that the family recordings and the clips you
// imitate are different things. So the tag stays; it's just one word now, with
// the sentence it replaced kept as the button's tooltip. Nothing is lost, it
// just no longer costs two wrapped lines under every card.
function setSrc(tag, full) {
  playNote.textContent = tag;
  playBtn.title = full ? full + ' — hear it (p)' : 'Hear it (p)';
  syncBarNotes();
}

// One lane in the bar, two things that want it: the provenance tag, and the
// live say-it status ("listening…", "the model", "you"). The status wins while
// it has something to say.
function syncBarNotes() {
  playNote.hidden = !!sayNote.textContent;
}

function updatePlayRow() {
  cardBar.hidden = !current;
  if (!current) { playBtn.disabled = true; setSrc('', ''); return; }
  const src = modelSrc();
  if (src) {
    playBtn.disabled = false;
    playBtn.innerHTML = '&#128266;';
    if (!flipped && current.llSrc) {
      setSrc('volunteer', 'real voice — a volunteer, not the family');
    } else {
      setSrc('studio', flipped && current.exampleSrc
...
const sayTipBtn = document.getElementById('sayTipBtn');
const sayTipBox = document.getElementById('sayTipBox');
const rhythmTipEl = document.getElementById('rhythmTip');
const sayNote = document.getElementById('sayNote');
const recBtn = document.getElementById('recBtn');
const cmpBtn = document.getElementById('cmpBtn');
const autoCmpBox = document.getElementById('autoCmpBox');
let comparing = false;

// Whether stopping a recording immediately plays the comparison. On by
// default — the research this feature is built on is specifically about
// hearing the two back to back, and a compare you have to remember to press
// is one you'll skip. But it commits you to ~9 seconds of playback on every
// single take, so it's a preference rather than a rule.
//
// Device-local, deliberately not in _sync_js.py's SYNC_KEYS: that carries
// learning data (schedules, deck edits, flags), and this is a UI preference
// like sessionSize and showTranslations, which are also per-device.
const AUTO_CMP_KEY = 'bahasa:autoCompare:v1';

function autoCompareOn() {
  try { return localStorage.getItem(AUTO_CMP_KEY) !== '0'; } catch (e) { return true; }
}

autoCmpBox.checked = autoCompareOn();
autoCmpBox.addEventListener('change', () => {
  try { localStorage.setItem(AUTO_CMP_KEY, autoCmpBox.checked ? '1' : '0'); } catch (e) {}
  // Unticking mid-compare should stop the one that's already playing, not
  // just affect the next take.
  if (!autoCmpBox.checked && comparing) {
    // recAbortCompare only silences the recording — the model clip is the
    // page's own audio, so stopping it has to happen here.
    recAbortCompare();
    stopAudio();
    comparing = false;
    sayNote.textContent = recHasClip() ? 'recorded' : '';
    sayNote.className = 'src';
    updateSayRow();
  }
});

// "Listen for" is per-card and worth keeping, but it doesn't have to occupy a
// 33px header on every card to be one tap away.
function sayTipOpen() { return !!tipsLoad()['say']; }
function applySayTipOpen() {
  const open = sayTipOpen() && !sayTipBtn.hidden;
  sayTipBox.hidden = !open;
  sayTipBtn.classList.toggle('on', open);
}
...
function updateSayRow() {
  ...
  recBtn.hidden = !show;
  // Compare appears only once there is something to compare against, which is
  // exactly when it stops being disabled anyway — it used to sit greyed out on
  // every card next to an "auto" checkbox that's a preference, not a control.
  ...
  cmpBtn.hidden = !(show && recHasClip());
  const tip = !!(show && current && current.sayTip);
  sayTipBtn.hidden = !tip;
  if (tip) sayTipBox.innerHTML = current.sayTip;
  applySayTipOpen();
  ...
  // compare loop. But it's the same paragraph on all 997 cards, so it retires
  // on the same rule as the keyboard hint: once you've plainly read it.
  rhythmTipEl.hidden = !(flipped && current && current.example) || staticTipsRetired();
  syncBarNotes();
}
...
async function toggleRecord() {
  if (recIsRecording()) {
    ...
    sayNote.textContent = url ? 'recorded' : 'nothing recorded';
    sayNote.className = 'src';
    updateSayRow();
    if (url && autoCompareOn()) compareSay();
    return;
  }
...
async function compareSay() {
  ...
  const finished = await recCompare(playModel, step => {
    if (step === 'model') { sayNote.textContent = 'the model'; sayNote.className = 'src model'; }
    else if (step === 'you') { sayNote.textContent = 'you'; sayNote.className = 'src you'; }
    else { sayNote.textContent = 'again?'; sayNote.className = 'src'; }
    syncBarNotes();
  });
...
// Everything in the bar lives inside the card, which flips on click — so each
// control stops propagation, the same way the "Heard in the recording" play
// button already does.
recBtn.addEventListener('click', e => { e.stopPropagation(); toggleRecord(); });
cmpBtn.addEventListener('click', e => { e.stopPropagation(); compareSay(); });
sayTipBtn.addEventListener('click', e => {
  e.stopPropagation();
  const state = tipsLoad();
  state['say'] = !sayTipOpen();
  tipsSave(state);
  applySayTipOpen();
});

// ---- Collapsed-by-default text --------------------------------------------
// Expanded state is a preference, not per-card: open "Heard in the recording"
// once and it stays open as you move through the deck.
const HEARD_KEY = 'bahasa:heardOpen:v1';
function heardOpen() { return !!srsLoad(HEARD_KEY).open; }
function setHeardOpen(v) { srsSave(HEARD_KEY, { open: !!v }); }
function applyHeardOpen(el) {
  const open = heardOpen();
  el.classList.toggle('open', open);
  const body = el.querySelector('.heardBody');
  if (body) body.hidden = !open;
}

__TIPS_JS__

// Text that is identical on every card earns its place only until you've read
// it. Both the keyboard hint and the sentence-rhythm explainer retire on this
// rule; per-card text ("Listen for", the example, the recording) never does.
function staticTipsRetired() {
  return Object.keys(srsLoad(REVIEWLOG_KEY)).length >= 10;
}

function renderHint() {
  const el = document.getElementById('hint');
  // Retire it once you've plainly learned the controls. Until then it's
  // useful; after that it's just height between the card and the buttons.
  if (staticTipsRetired()) { el.hidden = true; return; }
  el.hidden = false;
  el.textContent = srsIsTouch()
...
document.getElementById('moreBtn').addEventListener('click', () => {
  // Both states are cheap to recompute and only matter while the menu is up,
  // so they're derived on open rather than kept in sync on every card.
  if (togglePanel('morePanel')) {
    document.getElementById('removeBtn').disabled = !current;
    updateRestoreCount();
  }
});
